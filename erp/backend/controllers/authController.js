...
export const inviteUser = async (req, res) => {
    ...
    // Helper to send email (defined inside to access res easily, or could be separate)
    ...
    try {
        db.get("SELECT * FROM users WHERE email = ?", [email], async (err, row) => {
            ...
            stmt.run(id, name || 'Invited User', email, password_hash, role, status, finalDepartment, async function (err) {
                if (err) return res.status(500).json({ error: err.message });
...
export const register = (req, res) => {
    ...
    db.get("SELECT * FROM users WHERE email = ?", [email], (err, row) => {
        ...
        if (row.status === 'Invited') {
            ...
            const stmt = db.prepare("UPDATE users SET name = ?, password_hash = ?, status = ? WHERE id = ?");
            stmt.run(name, password_hash, status, row.id, function (updateErr) {
